...
async function main() {
  ...
  for (const item of items) {
    ...
    if (result.score < MIN_SCORE_TO_SHOW) {
      ...
      // Keep near-misses (score >= 15) for debugging/tuning visibility --
      // full low-score jobs (score < 15) aren't worth cluttering the file with.
      if (result.score >= 15) {
        nearMisses.push({ title, score: result.score, reasons: result.reasons });
      }
      continue;
    }
  ...
  const output = {
    ...
    near_misses: nearMisses.sort((a, b) => b.score - a.score).slice(0, 15),
    jobs: results
  };
