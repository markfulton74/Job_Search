...
export function scoreJob({ title, body, jobProfile, candidateProfile }) {
  ...
  const addBody = (pts, label) => { bodyScore += pts; reasons.push(`${pts > 0 ? "+" : "-"} ${label}`); };

  // Preferred roles
  for (const [role, weight] of Object.entries(PREFERRED_ROLES)) {
    if (titleLower.includes(role)) addTitle(weight, role);
    else if (fullText.toLowerCase().includes(role)) addBody(Math.round(weight * 0.5), role);
  }

  // Penalised professions
  for (const [prof, weight] of Object.entries(PENALISED_PROFESSIONS)) {
    if (titleLower.includes(prof)) addTitle(weight, prof);
    else if (fullText.toLowerCase().includes(prof)) addBody(Math.round(weight * 0.5), prof);
  }

  // Mobility signals (not title-weighted -- these are usually in the body/benefits section)
  for (const { re, points } of [...MOBILITY_POSITIVE, ...MOBILITY_NEGATIVE]) {
    if (re.test(fullText)) {
      addBody(points, re.source.slice(0, 40));
    }
  }
  ...
  if (jobProfile?.years_experience_required != null && candidateProfile?.years_experience_total != null) {
    ...
    const diff = have - req;
    if (diff >= 0 && diff <= 10) {
      addBody(15, `experience matches (${have}y vs ${req}y required)`);
    } else if (diff > 10) {
      addBody(-5, `may be overqualified (${have}y vs ${req}y required)`);
    } else {
      addBody(-30, `below required experience (${have}y vs ${req}y required)`);
    }
  }
  ...
  if (jobProfile?.education_field) {
    ...
    if (EDUCATION_BOOST_FIELDS.some((f) => field.includes(f))) {
      addBody(10, `education field aligned (${jobProfile.education_field})`);
    } else if (EDUCATION_PENALTY_FIELDS.some((f) => field.includes(f))) {
      addBody(-20, `education field mismatch (${jobProfile.education_field})`);
    }
  }
  if (jobProfile?.degree_required === "master" || jobProfile?.degree_required === "phd") {
    if (candidateProfile?.education?.highest_level && !["master", "phd"].includes(candidateProfile.education.highest_level)) {
      addBody(-20, `requires ${jobProfile.degree_required}'s degree`);
    }
    ...
    if (candidateProfile?.education?.highest_level && !["bachelor", "master", "phd"].includes(candidateProfile.education.highest_level)) {
      addBody(-15, "requires bachelor's degree");
    }
  } else if (jobProfile?.degree_required === "equivalent_experience_accepted") {
    addBody(15, "degree can be substituted by experience -- good fit");
  }
  ...
  if (jobProfile?.management_level && candidateProfile?.management_level) {
    ...
    if (jobIdx !== -1 && candIdx !== -1) {
      ...
      if (gap === 0) addBody(10, "management level matches");
      else if (gap === 1) addBody(3, "slightly senior for this level");
      else if (gap >= 2) addBody(-15, "likely overqualified for this level");
      else if (gap <= -2) addBody(-25, "likely underqualified for this level");
    }
  }

  const weighted = (titleScore * TITLE_WEIGHT * 2.5) + (bodyScore * BODY_WEIGHT);
